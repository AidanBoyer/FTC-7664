...
const float ticksPerRevolution = 757.12;
...
{
	clearDebugStream();
	int ticksToMove = (int)ticksPerRevolution;
	nMotorEncoder[EncoderTestMotor] = 0;
	int encoderNow = nMotorEncoder[EncoderTestMotor];
	long encoderStart = nMotorEncoder[EncoderTestMotor];
	long encoderDestination = encoderStart + ticksToMove;
  writeDebugStreamLine("Encoder value is %i", encoderNow);

	while(abs(encoderDestination - encoderNow) > 1)
	{
		encoderNow = nMotorEncoder[EncoderTestMotor];
		motor[EncoderTestMotor] = (encoderDestination - encoderNow) / 5;
	}
	motor[EncoderTestMotor] = 0;
	writeDebugStreamLine("Amount changed is %i", nMotorEncoder[EncoderTestMotor] - encoderStart);
}
