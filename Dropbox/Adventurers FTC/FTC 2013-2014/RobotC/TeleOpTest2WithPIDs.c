...
waitForStart();
	while(true)                        // infinite loop:
	{
  	...
  	motor[ScoopLeft] = scaledJoyDrive(joystick.joy2_y2);
		...
		if(joy2Btn(5) == 1)                  // If Joy1-Button1 is pressed:
		{
			motor[FlagRaise] = -20;
		} // end if
		else
		{
			if(joy2Btn(6) == 1)                  // If Joy1-Button1 is pressed:
			{
				motor[FlagRaise] = 20;
			} // end if
			else
			{
				if(joy2Btn(7) == 1)                  // If Joy1-Button1 is pressed:
				{
					motor[FlagRaise] = -100;
				} // end if
				else
				{
					if(joy2Btn(8) == 1)                  // If Joy1-Button1 is pressed:
					{
						motor[FlagRaise] = 100;
					} // end if
					else
