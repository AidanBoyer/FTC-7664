...
int scaledJoyDrive(int i_joyY)
{
	...
	if (abs(joyY)<joyDead) { scaledY = 0; }
  else { scaledY = (joyY>0) ? (int)((joyY + Yoffset) / (127 + Yoffset) * 100) : (int)((joyY - Yoffset) / (-128 - Yoffset) * -100); }
...
task main()
{
	...
	while(true) // infinite loop:
	{
  	...
  	motor[LeftRearDrive] =  scaledJoyDrive(joystick.joy1_y1);
