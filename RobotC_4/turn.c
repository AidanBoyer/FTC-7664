...
void driveMotors(float rf, float rr, float lf, float lr)
{
	...
	motor[RightFrontDrive] = (rf * multiplyer) * 104 / 100;
	motor[RightRearDrive] = (rr * multiplyer) * 104 / 100;
...
void turn(int angle, int speed)
{
	...
	const float inchesToTravel = abs(angle) * inchesPerFullTurn / 360;
	const float ticksToTurn = inchesToTravel * ticksPerRevolution / (wheelDiameter * PI); // final number of
	// encoder ticks to travel
	clearEncoders();

	while(abs(((((nMotorEncoder[RightFrontDrive] + nMotorEncoder[RightRearDrive]) / 2) ) - ((nMotorEncoder[LeftFrontDrive] + nMotorEncoder[LeftRearDrive]) / 2))) < ticksToTurn)
	...
	// calculated from the specified number of feet to move.
	{
		...
		driveMotors(direction*speed, direction*speed, direction*speed*-1, direction*speed*-1);
	}
}

void goSetDist(float dist, int speed)
{
...
void goUntilIRBonSide(int Speed)
{
	do
	{
